# Modified version of MVPAClassify.py to include searchlight analysis
...
def get_cross_validator(param, clf, partitioner, indent=0):
	...
	# let output be meaned across samples if doing searchlight
	if param['searchlight']:
		cv_kwargs['postproc'] = mean_sample()
	
	# tabulation
	if param['tabulate_results']:
		def callback(data, node, result):
			for attr in param['tab_attrs'].keys():
				result.sa[attr] = data[data.sa.partitions == 2].sa[attr][0:1]
		cv_kwargs['callback'] = callback
...
def get_classification_results(param, ds, res, cv, mean_control, indent=0):
	...
	if param['tabulate_results']:
		tab_attrs = param['tab_attrs']
		tab_dim = tuple([len(attr) for attr in tab_attrs.values()])
		result['tab'] = { 'all': np.zeros(tab_dim),
						  'correct': np.zeros(tab_dim),
						  'dims': tab_attrs.keys(),
						  'rows': tab_attrs }

		for indices in itertools.product(*[range(i) for i in tab_dim]):
			vals = map(lambda x,y: [x[y]], tab_attrs.values(), indices)
			criteria = dict(zip(tab_attrs.keys(), vals))
			res_sliced = res[criteria]
			result['tab']['all'][indices] = res_sliced.nsamples
			result['tab']['correct'][indices] = sum(res_sliced.samples)

		if param['matchedcrossclassify']:
			# multiply by 2 since each result is actually a forward and a backward fold
			result['tab']['all'] *= 2
			result['tab']['correct'] *= 2

	return result
	
def do_searchlight(param, ds, cv, indent=0):
	# source: http://www.pymvpa.org/tutorial_eventrelated_searchlight.html

	radius = param['searchlight_radius']
	status('running searchlight with radius: %i' % (radius), indent=indent)

	# construct the searchlight operation
	sl = Searchlight(cv,
					 IndexQueryEngine(voxel_indices=Sphere(radius), # spatial radius
					 				  event_offsetidx=Sphere(0) # temporal radius
					 				  ),
	...
	res.samples = 1-res.samples

	# map into image and save
	ts = res.a.mapper.reverse1(res.samples[0])
	# need to put the time axis last for export to NIfTI
	ts = np.rollaxis(ts, 0, 4)
	ni = nb.Nifti1Image(ts, ds.a.imgaffine).to_filename(param['path_result'])
	return ni
...
def classify_one(param, ds, name, mean_control=False, indent=0):
	...
	if not param['searchlight']:
		...
		result = do_searchlight(param, ds, cv, indent=indent+1)
...
def classify_mask(param, data, mask=None, mask_bootstrap_idx=None, indent=0):
	...
	# if doing searchlight, result is already saved.
	if not param['searchlight']:
