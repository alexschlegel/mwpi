function res = Analysis_20170617_ANOVA_all
% Analysis_20170617_ANOVA_all.m
% compute ANOVAs on classification accuracy between same, related, and
% unrelated image/percept intersections. Use all trials, not just correct.

global strDirAnalysis

% create directory for analysis results
strNameAnalysis = '20170617_anova_all';
...
sROIRes = load(PathUnsplit(DirAppend(strDirAnalysis, '20170615_roimvpa_all'),'result','mat'));
res.roi = DoANOVA(sROIRes);

% 2) Cross classification
sCCRes = load(PathUnsplit(DirAppend(strDirAnalysis, '20170616_roiccmvpa_all'),'result','mat'));
...
save(strPathOut,'res');
	...
	function res = DoANOVA(sResult)
		for kS = 1:2
			...
			cMask = arrayfun(@(kM) strjoin(cMask(kM,:),'-'), (1:size(cMask,1))', 'uni', false);
			res.mask = cMask;
