...
%       extra_targets:  (struct()) scalar or nSubject x 1 struct of extra
%                       targets (see help for MVPAClassify.m)
...
%get a target/chunk/zscore set for each classification
    ...
    if isfield(sPath.opt_extra, 'extra_targets')
        assert(isstruct(sPath.opt_extra.extra_targets), 'Extra targets must be struct array.');
        sExtraTargets = sPath.opt_extra.extra_targets;
    else
        sExtraTargets = struct();
    end
	
	[sData.cPathDataROI,sData.cNameROI,cTarget,kChunk,kZScore,sExtraTargets] = ...
        FillSingletonArrays(sData.cPathDataROI,sData.cNameROI,cTarget,kChunk,kZScore,sExtraTargets);
	...
	%make sure each parameter is nMask x 1 on the inside
        ...
        sExtraTargets = arrayfun(@(s) repmat(s,[nMask,1]),sExtraTargets,'uni',false);
	
	%reshape to nMask x nSubject
		[sData.cPathDataROI,sData.cNameROI,cTarget,kChunk,kZScore,sExtraTargets] = ...
            varfun(@(c) cat(2,c{:}),sData.cPathDataROI,sData.cNameROI,cTarget,kChunk,kZScore,sExtraTargets);
