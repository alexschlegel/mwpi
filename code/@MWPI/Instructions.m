...
strTimeEstimate = num2str(roundn((MWPI.Param('practice','block','time') + 1500) ...
...
strRetention = ['<size:' szHeader '>Instructions (cont.):\n</size>' ...
	'After the frame disappears, you will see another image in the center of the screen. Over the next ' ...
	secRetention ' seconds, it will grow or shrink momentarily a few times. '...
...
strTest = ['<size:' szHeader '>Instructions (cont.):\n</size>' ...
	...
	'of the screen. You will have ' secTest ' seconds to indicate where you now see the image ' ...
	'you remembered with the <color:yellow>Y</color>, <color:green>A</color>, ' ...
	'<color:blue>X</color>, or <color:red>B</color> button.'];
...
strExample = ['Next, you will do an example trial.\n There will be some hints in ' ...
	'<color:' colHint '>' colHint '</color> to guide you which will not ' ...
...
for kT = 1:numel(hintFigTexture)
	exp.Show.Text(['<color:' colHint '>Remember</color>'], ...
		[2*hintOffset,2*hintOffset - 0.6 * hintSzVA], 'window', hintFigTexture{kT});
	exp.Show.Image(hintFig, [2*hintOffset,2*hintOffset], 'window', hintFigTexture{kT}, ...
